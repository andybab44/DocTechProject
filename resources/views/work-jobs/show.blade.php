...
<div class="max-w-xl space-y-6">
    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-4">
        ...
        <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            <dt class="text-gray-500 font-medium">{{ __('app.work_job.details_requested_by') }}</dt>
            <dd class="text-gray-800">{{ $workJob->doctor->name }}</dd>
            <dt class="text-gray-500 font-medium">{{ __('app.work_job.details_assigned_to') }}</dt>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <h2 class="text-sm font-semibold text-gray-700 mb-3">{{ __('app.work_job.update_status') }}</h2>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-semibold text-gray-700">{{ __('app.work_job.attachments') }}</h2>
            <button type="button" onclick="openUploadModal()"
                    class="px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition flex items-center gap-1.5">
        ...
        @if($workJob->attachments->isEmpty())
            <p class="text-sm text-gray-400">{{ __('app.work_job.no_attachments') }}</p>
        @else
        <ul class="divide-y divide-gray-100">
            ...
            <li class="flex items-center justify-between py-2.5 text-sm">
                <button type="button"
                    ...
                    <span class="text-gray-800 font-medium group-hover:text-indigo-600 transition">{{ $attachment->original_name }}</span>
    ...
    <div class="relative flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-6">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-base font-semibold text-gray-800">{{ __('app.work_job.attach_file') }}</h3>
            ...
            <form id="uploadForm" method="POST" action="{{ route('work-jobs.attachments.store', $workJob) }}"
                ...
                {{-- Drop zone --}}
                <div id="dropZone"
                     class="relative flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center cursor-pointer transition hover:border-indigo-400 hover:bg-indigo-50"
                ...
                <div id="filePreview" class="hidden mt-3 flex items-center gap-3 rounded-lg bg-indigo-50 border border-indigo-100 px-3 py-2.5">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-indigo-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button type="button" onclick="clearFile()" class="text-gray-400 hover:text-red-500 transition flex-shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>
                ...
                <div class="mt-5 flex justify-end gap-3">
                    ...
                    <button id="uploadSubmitBtn" type="submit" disabled
                            class="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg transition hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
...
<div id="downloadModal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
    ...
    <div class="relative flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-base font-semibold text-gray-800">{{ __('app.work_job.download') }}</h3>
                <button type="button" onclick="closeDownloadModal()" class="text-gray-400 hover:text-gray-600 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                    </svg>
                </button>
            ...
            <div class="flex items-start gap-4 mb-6">
                ...
                <div class="min-w-0">
                    <p id="dlFileName" class="text-sm font-semibold text-gray-800 break-all"></p>
                    <p id="dlFileMeta" class="text-xs text-gray-500 mt-0.5"></p>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <a id="dlLink" href="#"
                   class="px-4 py-2 text-sm text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="relative flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
            <div class="flex items-start gap-4 mb-5">
                ...
                <div>
                    <h3 class="text-base font-semibold text-gray-800">{{ __('app.work_job.remove_attachment') }}</h3>
                    <p class="text-sm text-gray-500 mt-1" id="deleteConfirmText"></p>
                </div>
            </div>
...
function clearFile() {
    ...
    document.getElementById('dropZoneLabel').innerHTML = '{!! addslashes(__('app.work_job.drop_or_browse', ['link' => '<span class="font-medium text-indigo-600">' . __('app.work_job.click_to_browse') . '</span>'])) !!}';
}
...
function openDeleteModal(id, name) {
    document.getElementById('deleteConfirmText').innerHTML = '{{ __('app.work_job.remove_confirm', ['name' => '<span id="deleteFileName" class="font-medium text-gray-700"></span>']) }}';
