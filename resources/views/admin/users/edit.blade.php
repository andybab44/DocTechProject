...
<div class="mb-6 flex items-center gap-3">
    <a href="{{ route('admin.users.index') }}" class="text-stone-400 hover:text-stone-600 transition">{{ __('app.users.btn_back') }}</a>
    <h1 class="text-2xl font-semibold text-stone-800">{{ __('app.users.edit_title') }}</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}" novalidate>
        ...
        <div class="mb-4">
            <label for="password" class="block text-sm font-medium text-stone-700 mb-1">
                {{ __('app.users.field_password_new') }} <span class="text-stone-400 font-normal">{{ __('app.users.field_password_keep') }}</span>
            </label>
        ...
        {{-- Confirm Password --}}
        <div class="mb-6">
            <label for="password_confirmation" class="block text-sm font-medium text-stone-700 mb-1">{{ __('app.users.field_password_new_confirm') }}</label>
        ...
        <div class="flex items-center gap-3">
            <button type="submit"
                class="px-5 py-2 bg-teal-700 hover:bg-teal-800 text-white text-sm font-medium rounded-md transition">
                {{ __('app.users.btn_save') }}
            </button>
            <a href="{{ route('admin.users.index') }}"
               class="px-5 py-2 text-sm text-stone-600 border border-stone-300 rounded-md hover:bg-stone-50 transition">
