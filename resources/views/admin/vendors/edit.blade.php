...
@section('title', __('app.vendors.edit_title'))

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.vendors.index') }}" class="text-sm text-teal-700 hover:underline">← {{ __('app.vendors.title') }}</a>
    <h1 class="text-2xl font-semibold text-stone-800 mt-1">{{ __('app.vendors.edit_title') }}: {{ $vendor->name }}</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.vendors.update', $vendor) }}" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="notes" rows="3"
                      class="w-full rounded-md border border-stone-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400
                             @error('notes') border-red-400 @enderror">{{ old('notes', $vendor->notes) }}</textarea>
