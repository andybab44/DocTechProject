...
<div class="mb-6 flex items-center gap-4">
    ...
    <h1 class="text-2xl font-semibold text-gray-800">{{ __('app.licenses.create_title') }}</h1>
</div>
...
<div class="max-w-lg bg-white rounded-xl shadow p-6">
    <form method="POST" action="{{ route('admin.licenses.store') }}" class="space-y-5">
        ...
        {{-- User --}}
        <div>
            <label for="user_id" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.licenses.field_user') }} <span class="text-red-500">*</span></label>
            <select id="user_id" name="user_id" required
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 @error('user_id') border-red-400 @enderror">
            ...
            @enderror
            @if($users->isEmpty())
            <p class="mt-1 text-xs text-gray-400">{{ __('app.licenses.no_users_available') }}</p>
            @endif
        </div>
