    ...
    <div class="flex items-center gap-3">
        @if($appointment->status === \App\Enums\AppointmentStatus::Scheduled)
            ...
            <a href="{{ route('appointments.edit', $appointment) }}"
               class="inline-flex items-center text-sm bg-white border border-gray-300 rounded-md px-4 py-2 hover:bg-gray-50 transition">
...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl space-y-4">
    <dl class="divide-y divide-gray-100 text-sm">
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_patient') }}</dt>
            <dd class="col-span-2">
        ...
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_doctor') }}</dt>
            <dd class="col-span-2 text-gray-800">{{ $appointment->doctor->name }}</dd>
        </div>
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_date') }}</dt>
            <dd class="col-span-2 text-gray-800">{{ $appointment->scheduled_at->format('d M Y H:i') }}</dd>
        </div>
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_status') }}</dt>
            <dd class="col-span-2">
        ...
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_work_job') }}</dt>
            <dd class="col-span-2">
                <a href="{{ route('work-jobs.show', $appointment->workJob) }}" class="text-indigo-600 hover:underline">
        ...
        <div class="py-3 grid grid-cols-3">
            <dt class="font-medium text-gray-500">{{ __('app.appointments.col_notes') }}</dt>
            <dd class="col-span-2 text-gray-700 whitespace-pre-wrap">{{ $appointment->notes }}</dd>
        </div>
        @endif
    </dl>
</div>
